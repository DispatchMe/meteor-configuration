 ...
 *   
 * @param  {Array<Object>} entities Array of entity objects with "type" and "id" properties
...
Configuration.getForEntities = function(entities, options) {
  ...
  var docsToFindById = [];
  entities.forEach(function(entity) {
    var extendList = Configuration._resolveInheritance(entity.type, entity.id, options);
    inheritanceTrees.push(extendList);
    // We want to find this one, in addition to all the things it needs to inherit from.
    docsToFindById.push(entity.type + '_' + entity.id);
  ...
  entities.forEach(function(entity, index) {
    results.push(Configuration._extendEntity(entity.type, entity.id, inheritanceTrees[index], documentCache).config);
  });
